...
void cColectivo_nuevo::Subir_Personas()
{
	for (int i = 0; i < 9; i++)
	{
		...
		{
			if (Parada_Actual->getListaPersonas()->getItem(j)->getDestino() == ramal->getLista()->getItem(i)->getNombre() && Parada_Actual->getListaPersonas()->getItem(i)->getSilla_ruedas() == true)
			{
